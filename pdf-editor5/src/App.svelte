<script>
	...
	function addTextField(text = 'Metin alanı') {
		const id = genID();
		fetchFont(currentFont);
		const object = {
			id,
	...
	async function savePDF() {
		...
		saving = true;
		try {
			//BEN await save(pdfFile, allObjects, pdfName, pagesScale);
	...
	function windowOns(e) {
		e.preventDefault();
	}
</script>

<svelte:window ondragenter={windowOns} ondragover={windowOns} ondrop={onUploadPDF} />
...
<main class="flex flex-col items-center py-16 bg-gray-100 min-h-screen">
	...
	>
		<input type="file" name="pdf" id="pdf" onchange={onUploadPDF} class="hidden" />
		<label
      ...
      md:mr-4"
		>
			<input type="file" id="image" name="image" class="hidden" onchange={onUploadImage} />
			<label
	...
	{:else}
		<div class="w-full flex-grow flex justify-center items-center">
			<span class=" font-bold text-3xl text-gray-500">Buraya PDF dosyanızı bırakabilirsiniz</span>
		</div>
	{/if}
</main>
